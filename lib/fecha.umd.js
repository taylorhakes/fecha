(function (global, factory) {
  ...
  const setGlobalI18n = (i18n) => Object.assign(globalI18n, i18n);
  ...
  const setGlobalMasks = (masks) => Object.assign(globalMasks, masks);
   ...
   */
  const format = (dateObj, mask=globalMasks['default'], i18n=globalI18n) => {
   ...
   */
  const parse = (dateStr, format, i18n=globalI18n) => {
    ...
    newFormat = regexEscape(newFormat).replace(token, function ($0) {
      ...
      const [field, regex, _, requiredField] = info;
    ...
    for (let i = 1; i < matches.length; i++) {
      const [field, regex, parser, requiredField] = parseInfo[i-1];
    ...
    const dateWithoutTZ = new Date(dateInfo.year, dateInfo.month, dateInfo.day,
        dateInfo.hour, dateInfo.minute, dateInfo.second, dateInfo.millisecond);
    ...
    return new Date(Date.UTC(dateInfo.year, dateInfo.month, dateInfo.day,
        dateInfo.hour, dateInfo.minute, dateInfo.second, dateInfo.millisecond));
  };
  ...
  exports.setGlobalMasks = setGlobalMasks;
